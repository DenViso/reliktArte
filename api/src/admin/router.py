...
def extract_docx_content(file_path: Path):
    ...
    if not DOCX_AVAILABLE:
        return "Опис відсутній", [{"value": "python-docx не встановлено"}], None, False, False, 0
    
    if not file_path.exists():
        return "Файл відсутній", [{"value": "Файл опису відсутній"}], None, False, False, 0
    ...
    try:
        ...
        if not all_paragraphs:
            return "Опис порожній", [{"value": "Документ порожній"}], None, False, False, 0
        ...
        return f"Помилка: {str(e)}", [{"value": "Помилка читання"}], None, False, False, 0
...
async def import_doors_task(session: AsyncSession, category_id: int) -> Dict[str, int]:
    ...
    if not catalog_path.exists():
        return {"imported": 0, "updated": 0, "skipped": 0, "photos_added": 0, "error": "Каталог не знайдено"}
...
async def import_mouldings_task(session: AsyncSession, category_id: int) -> Dict[str, int]:
    """Імпорт лиштв (плоска структура - без класів)"""
    ...
    # Читаємо папки безпосередньо з mouldings/ (без класів)
